...
then
    rkdevtoola=" - 'rkdeveloptool' not installed (used for uefi spi flashing). Install it (some ditros may not have it yet, in that case you'd need to <a href='https://opensource.rock-chips.com/wiki_Rkdeveloptool'>compile it</a>"
    export requiredep=1
fi
...
zenity --title "workli" --info --ok-label="Next" --text "WoRKli, made by JoJo Autoboy#1931\n\n Not really based off of Mario's WoR Linux <a href='https://worproject.com/guides/how-to-install/from-other-os'>guide</a>"
...
zenity --question --title="workli" --text "Do you want to flash the uefi bootloader? (manditory for first time run)"
    ...
    [1])
        ...
        case $? in
            [0])
                ...
                export fdisk=$(parted -l | grep "Disk /dev*" | grep -v loop | sort | gawk '{ printf "FALSE""\0"$0"\0" }' | xargs -0 zenity --list --title="workli" --text="What /dev/* is your SD card?" --radiolist --multiple --column ' ' --column 'Disks' --extra-button "Rescan")
...
dwnopt=$(zenity --question --title="workli" --text "Do you want:\n\n(1) this script to download a esd directly from microsoft (recommended, release versions only, gives more type options)\n(2) generate iso with uupdump (slower, for insider builds)\n(3) use your own iso/esd or a previously generated/downloaded one" --switch --extra-button "1" --extra-button "2" --extra-button "3" )
...
if [[ $requiredep == *"1"* ]]; then
    zenity --title "workli" --info --ok-label="Exit" --text "Dependances\n\n$aria\n\n$curle"
    exit 1
else
...
winlanguage=$(echo $getcatalogxml | xmlstarlet sel -t -v /MCT/Catalogs/Catalog/PublishedMedia/Files/File/Language | zenity --list --title="workli" --text="What windows language do you want?" --column 'Windows Language' --width=300 --height=300)
...
zenity --title "workli" --info --ok-label="Continue" --text "The uupdump script will now be executed\nIts output will be in the terminal where this script was ran"
...
if [[ $? -ne 0 ]]; then
    ...
    zenity --title "workli" --info --ok-label="Continue" --text "The uupdump script suceeded!"
fi
...
export fdisk=$(parted -l | grep "Disk /dev*" | grep -v loop | sort | gawk '{ printf "FALSE""\0"$0"\0" }' | xargs -0 zenity --list --title="workli" --text="What /dev/* is your drive?" --radiolist --multiple --column ' ' --column 'Disks' --extra-button "Rescan")
...
zenity --title "workli" --info --ok-label="Done!" --text "Booting\n\n1. Connect the drive and other peripherals to your board then boot it up.\n\n2. Assuming everything went right in the previous steps, the device will boot up to a PE enviroment were it will do some configuring and then reboot to hopefully a full windows install.\n\nAll done :)\nThanks for using WoRKli"
